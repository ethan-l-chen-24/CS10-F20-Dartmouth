/**
 * DotTreeGUIExtension
 * An extension on the DotTreeGUI class to include "user input" mode, as well as test mode that shows what points have
 * undergone what tests
 *
 * @name -> Ethan Chen
 * @date -> October 7, 2020
 ...
 */
public class DotTreeGUIExtension extends DrawingGUI {
	...
	private PointQuadtreeExtension<Dot> tree = null;			// holds the dots
	...
	private boolean showTests = false; 				// shows the nodes being tested
	...
	public DotTreeGUIExtension() {
		super("dottreeExtension", width, height);
	}
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if(tree != null) { // if this is the first point clicked
				...
				tree = new PointQuadtreeExtension<Dot>(new Dot(x, y), 0, 0, width, height); // otherwise add to tree
			}
		}
		else if (mode == 'q') {
	...
	private void test0() {
		found = null;
		tree = new PointQuadtreeExtension<Dot>(new Dot(400,300), 0,0,800,600); // start with A
	...
	private void test1() {
		found = null;
		tree = new PointQuadtreeExtension<Dot>(new Dot(300,400), 0,0,800,600); // start with A
		tree.insert(new Dot(150,450)); // B
		tree.insert(new Dot(250,550)); // C
	...
	private void test2() {
		found = null;
		tree = new PointQuadtreeExtension<Dot>(new Dot(300,300), 0,0,800,600); // start with A
	...
	/** Allows the user to manually type in coordinates of points instead of clicking on them */
	private void userInput() {
		boolean cont; // continue adding dots as long as true
		Dot p = promptForPoint(); // get an initial dot
		tree = new PointQuadtreeExtension<Dot>(p, 0, 0, width, height); // make that dot the head of the tree
		cont = promptForExit(); // ask if wants to add more dots
		System.out.println();

		while(cont) { // as long as they answer yes/true for promptForExit()
			Dot newPoint = promptForPoint(); // get a new point
			tree.insert(newPoint); // add the point
			cont = promptForExit(); // ask if they want to exit
			System.out.println();
		}
		System.out.println("The blobs have been drawn");
	}

	/** Helper method that prompts from console to user to type in x and y coordinates */
	private Dot promptForPoint() {
		Scanner in = new Scanner(System.in); // user input
		int xCoord;
		int yCoord;
		System.out.println("What would you like as the x-coordinate of this point? ");
		xCoord = in.nextInt();
		System.out.println("What would you like as the y-coordinate of this point? ");
		yCoord = in.nextInt();

		return new Dot(xCoord, yCoord); // returns dot created from x and y values
	}

	/** Helper method that prompts from console to user to type in yes or no */
	private boolean promptForExit() {
		Scanner in = new Scanner(System.in); // user input
		char yn;
		System.out.println("Great! Would you like to add another dot?");
		yn = in.nextLine().charAt(0);

		if(yn == 'y' || yn == 'Y') { // returns true or false depending on what character they input - if y, true
			return true;
		} else {
			return false;
		}
	...
	public void handleKeyPress(char key) {
		...
		else if(key=='u') { // u - userInput, manually insert points through console
			userInput();
		}
		else if(key == 't') { // t - show test nodes in drawing
			showTests = !showTests;

			// ***
			// light grey means Geometry.circleIntersectsRectangle has been called on a point
			// dark grey means Geometry.pointInCircle has been called on a point
			// black means the point is within the circle

			// black implies dark grey and light grey
			// dark grey implies light grey
		}
		repaint();
	}
	...
	public void draw(Graphics g) {
		...
		if (mode == 'q') { // if in query mode
			...
			if(showTests) { // if t has been pressed
				g.setColor(Color.lightGray); // light gray is color of having been rectangle tested
				for(Dot d : tree.getCircleRectangleTested()) {
					g.fillOval((int)d.getX()-dotRadius, (int)d.getY()-dotRadius, 2*dotRadius, 2*dotRadius); // draw the dots
				}
				g.setColor(Color.GRAY); // gray is color of having been point in circle tested
				for(Dot d : tree.getPointInCircleTested()) {
					g.fillOval((int)d.getX()-dotRadius, (int)d.getY()-dotRadius, 2*dotRadius, 2*dotRadius); // draw the dots
				}
				if(found != null) {
					g.setColor(Color.BLACK); // draw the found dots black
				...
				} // SEE LINE 273 FOR EXPLANATION/HOW IT HELPS WITH TESTING
			}
		}
	}
	 ...
	 */
	public void drawTree(Graphics g, PointQuadtreeExtension<Dot> tree, int level) {
